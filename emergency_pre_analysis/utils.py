...
def estimate_node_pressures(wn, source_pressures):
    ...
    while queue:
        ...
        try:
            out_edges = [e for e in wn.get_links_for_node(current_node)
                         if wn.get_link(e).start_node_name == current_node]
        ...
        if node_id not in pressures:
            try:
                ...
                for link_name in wn.get_links_for_node(node_id):
                    link = wn.get_link(link_name)
                    other_node = (link.start_node_name if link.end_node_name == node_id
...
def reorient_edges_by_pressure(wn, source_pressures=None, verbose=True):
    """
    ...
    for atypical_node in atypical_nodes:
        try:
            outgoing_links = [link_name for link_name in wn.get_links_for_node(atypical_node)
                            if wn.get_link(link_name).start_node_name == atypical_node]
